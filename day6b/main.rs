...
fn santa_global(text: &str) -> u32 {
    let mut grid:Vec<u32> = vec![0;SIDE*SIDE];
    text.split("\n").for_each(|w| {
        santa(w.trim(), & mut grid);
    });
    grid.iter().filter(|&&f| f>0 ).sum::<u32>() as u32
}

fn get_coord_from_str(text: &str) -> (u32, u32) {
    let top_left:Vec<&str>=text.split(',').collect();
    let a= (top_left[0].parse::<u32>()).unwrap();
    let b= (top_left[1].parse::<u32>()).unwrap();
    (a, b)
}

fn change_light_rect(rect:&Vec<(u32,u32)>, grid: & mut Vec<u32>, command: &str) {
    ...
    for i in rect[0].0..=rect[1].0 {
        for j in rect[0].1..=rect[1].1 {
            match command {
                "T" => {
                    // Toggle
                    grid[i as usize*SIDE+j as usize]=2+grid[i as usize*SIDE+j as usize];
                },
                "B" => {
                    // Black
                    if  grid[i as usize*SIDE+j as usize] >0{
                        grid[i as usize*SIDE+j as usize]= grid[i as usize*SIDE+j as usize]-1;
                    }
                },
                "W" => {
                    // White
...
fn santa(text: &str, grid: & mut Vec<u32>) {
    let line:Vec<&str>=text.split(' ').collect();

    println!("Analysing len : {} of {:?} ", line.len(), line);

    // get coordinates
    let mut coord:Vec<(u32,u32)>=Vec::new();
    let coord_index = if line.len()==4 {0}else{1};
    let command = if line.len()==4 {"T"}else{if line[1]=="on"{"W"}else{"B"}};

    coord.push(get_coord_from_str(line[1+coord_index]));
    coord.push(get_coord_from_str(line[3+coord_index]));
    println!("\t{:?}", coord);

    change_light_rect(&coord, grid, command);
}
...
mod tests {
    ...
    #[test]
    fn fixed_tests() {
        dotest("turn on 0,0 through 999,999 ", 1000000); // Looking at         urrvucyrzzzooxhx	Real Pair z,z	Surrounded true : zzz
        dotest("turn off 0,0 through 999,999 ", 0); // Looking at         urrvucyrzzzooxhx	Real Pair z,z	Surrounded true : zzz
    }
    #[test]
    fn com_tests(){
        dotest(
            "turn on 102,229 through 674,529",
            63,
        );
    }
    ...
    #[test]
    fn prod_tests(){
        dotest("turn on 887,9 through 959,629
        ...
        turn off 102,229 through 674,529", 0);
    }
}
/* */
