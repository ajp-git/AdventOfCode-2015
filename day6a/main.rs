...
fn santa_global(text: &str) -> u32 {
    let mut grid:Vec<bool> = vec![false;SIDE*SIDE];
    text.split("\n").for_each(|w| {
        santa(w.trim(), & mut grid);
    });
    grid.iter().filter(|&&f| f ).count() as u32
}
...
fn change_light_rect(rect:&Vec<(u32,u32)>, grid: & mut Vec<bool>, command: &str) {
    ...
    for i in rect[0].0..=rect[1].0 {
        for j in rect[0].1..=rect[1].1 {
            match command {
                "T" => {
                    // Toggle
                    grid[i as usize*SIDE+j as usize]=!grid[i as usize*SIDE+j as usize];
                },
                "B" => {
                    // Black
                    grid[i as usize*SIDE+j as usize]=false;
                },
                "W" => {
                    // White
                    grid[i as usize*SIDE+j as usize]=true;
...
fn santa(text: &str, grid: & mut Vec<bool>) {
